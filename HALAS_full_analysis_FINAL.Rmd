...
render_etable <- function(title, basename, models, headers = NULL,
  ...
  # Neutraliser l'auto-déduction des headers si non fournis
  if (is.null(headers)) headers <- NA

  # FIX ABSOLU : empêcher etable de déduire des noms incorrects
  if (!is.na(headers[1])) {
    names(models) <- headers
  }

  # Construire la liste d'arguments pour etable
  args <- c(
    models,
    list(
      headers     = headers,
...
if (exists("m1_placebo_ols") && exists("m2_placebo_fe_cy") &&
  ...
  render_etable(
    ...
    models   = list(m1_placebo_ols, m2_placebo_fe_cy,
                    m3_placebo_fe_cy_ctrl, m4_placebo_fe_ay_ctrl),
    dict     = c(
      "y_log"="Log(Error)",
      "AQI_placebo"="AQI (Placebo)",
      "log_gdp"="Log(GDP)",
    ...
    tex      = FALSE,
    headers  = c("OLS placebo", "FE CY placebo",
                 "FE CY + controls", "FE AY + controls")
  )
}
