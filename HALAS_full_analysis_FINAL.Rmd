...
# y_log    = Log(Error) = log(APE + ε) → variable principale de l'article
...
vars_descrip <- c(
  ...
  "err_abs", "ape", "y_log", "y_asinh", "afe", "y_log_afe","mean_abs_err_jt", 
  # Pollution
...
if (all(c("ape","y_log") %in% names(data))) {
  ...
  labs(title = "Distribution of Log(Error)", 
       x = "Log(Error)", y = "Count") +
  theme_minimal() -> p2
...
###############################################################
# 2.15 Time evolution of the forecasting error (Log(Error))
###############################################################
...
ggplot(err_by_year, aes(x = as.factor(year), y = mean_ylog)) +
  ...
  geom_errorbar(aes(ymin = mean_ylog - 1.96*se,
                ...
                width = .15) +
  labs(title = "Average Log(Error) by Year (Europe)",
     x = "Year", y = "Average Log(Error)")
...
ggplot(err_by_year_ctry, aes(x = year, y = mean_ylog)) +
  ...
  facet_wrap(~ country, scales = "free_y") +
  labs(title = "Average Log(Error) by Year — FR, DE, GB, CH",
     x = "Year", y = "Average Log(Error)") +
  theme_minimal()
...
ggplot(plot_df, aes(x = AQI, y = y_log)) +
  ...
  labs(
    ...
    x = "AQI",
    y = "Log(Error)"      # = log(APE + ε) dans le papier
  ) +
  theme_minimal(base_size = 12) +
  theme(
...
#    - mean_ylog  : Log(Error) moyen par année (DV principale)
...
plot_err <- annual %>%
  transmute(year, series = "Average Log(Error)", y_plot = a + b * mean_ylog)
...
m3_fe_cy_ctrl <- feols(
  y_log ~ AQI + log_gdp + inflation_rate + unemployment_rate | country + year,
  data = df_iv
)

m4_fe_ay_ctrl <- feols(
  y_log ~ AQI + log_gdp + inflation_rate + unemployment_rate | id_analyst + year,
  data = df_iv
)
...
form_plm <- y_log ~ AQI + log_gdp + inflation_rate + unemployment_rate
...
library(mgcv)
gam_cy <- gam(
  y_log ~ s(AQI) + factor(country) + factor(year) +
           log_gdp + inflation_rate + unemployment_rate,
  data = df_iv,
  method = "REML"
)
...
if (!exists("f_controls")) {
  controls_macro <- intersect(
    c("log_gdp", "inflation_rate", "unemployment_rate"),
    names(data)
  )
  f_controls <- if (length(controls_macro) == 0)
    "1"
  else
    paste(controls_macro, collapse = " + ")
}
...
```{r}
model_2_2sls <- feols(
  y_log ~ log_gdp + inflation_rate + unemployment_rate |
  id_analyst + year |
  AQI ~ PBLH,
  data = df_iv
)
...
m3_placebo_fe_cy_ctrl <- feols(
  y_log ~ AQI_placebo + log_gdp + inflation_rate + unemployment_rate |
  country + year,
  data = df_iv_placebo
)
...
# 2.15 Average Log(Error) by Year (Europe + facets)
if (exists("err_by_year")) {
  p <- ggplot(err_by_year, aes(x = as.factor(year), y = mean_ylog)) +
    ...
    labs(title = "Average Log(Error) by Year (Europe)",
         x = "Year", y = "Average Log(Error)") + theme_minimal()
...
if (exists("plot_both") && exists("a") && exists("b")) {
  p <- ggplot(plot_both, aes(x = year, y = y_plot, color = series, linetype = series)) +
    ...
    scale_color_manual(name = "Indicator",
      values = c("AQI" = "#F28E2B", "Average Log(Error)" = "grey40")) +
    scale_linetype_manual(name = "Indicator",
      values = c("AQI" = "solid", "Average Log(Error)" = "dashed")) +
    scale_x_continuous(breaks = sort(unique(plot_both$year))) +
    scale_y_continuous(name = "Average AQI",
      sec.axis = sec_axis(~ (. - a) / b, name = "Average Log(Error)")) +
...
#    Wrapper do.call avec gestion explicite des headers
# -----------------------------------------------------------------
render_etable <- function(title, basename, models, headers = NULL,
  ...
  cat(
    html,
    file = file.path(pretty_dir, paste0(.sanitize(basename), ".html"))
  )
}
...
```{r}
# ============================================================
# HELPERS – validation simple des vcov pour fixest::etable
# ============================================================
...
.safe_vcov_for_models <- function(models, vc){
  ...
  # Cas formule simple : ~ country, ~ id_analyst + year
  if (inherits(vc, "formula")) return(vc)

  # Cas liste de formules : list(~ country, ~ country + year, ...)
  if (is.list(vc) && all(vapply(vc, inherits, logical(1), "formula"))) {
    if (length(vc) == length(models)) return(vc)
    return(NULL)
  }
...
```{r}
df_iv <- data
# Sauvegarde de la base principale avant les filtres IV
df_main <- data
```
...
if (exists("df_main")) {
  ...
  m2_fe_cy_export <- feols(
    y_log ~ AQI | country + year,
    data = df_main
  )

  m3_fe_cy_ctrl_export <- feols(
    y_log ~ AQI + log_gdp + inflation_rate + unemployment_rate | country + year,
    data = df_main
  )

  m4_fe_ay_ctrl_export <- feols(
    y_log ~ AQI + log_gdp + inflation_rate + unemployment_rate | id_analyst + year,
    data = df_main
  )
  ...
  out <- capture.output(
    etable(
      m1_ols_export,
      m2_fe_cy_export,
      m3_fe_cy_ctrl_export,
      m4_fe_ay_ctrl_export,
      vcov = list(
        ~ country,
        ~ country + year,
        ~ country + year,
      ...
      signif.code = if (exists("sc_stars")) sc_stars else c("***"=0.01, "**"=0.05, "*"=0.10),
      tex = FALSE
    )
  )

  html <- paste(c(
    "<h3>OLS and FE Models — Dependent Variable: Log(Error)</h3>",
    "<pre style='white-space: pre-wrap; font-family: Consolas, Menlo, monospace;'>",
    out,
    "</pre>"
  ), collapse = "\n")

  cat(html, file = file.path(pretty_dir, "Reg_OLS_FE.html"))
}
```
...
if (exists("m1_placebo_ols") && exists("m2_placebo_fe_cy") &&
    exists("m3_placebo_fe_cy_ctrl") && exists("m4_placebo_fe_ay_ctrl")) {
...
var_defs <- c(
  ...
  "y_log"="Log(Error), main dependent variable.",
...
vars_order <- c(
  # DV & erreurs
  "y_log", "ape", "err_abs", "y_asinh","mean_abs_err_jt", "afe",
  # Pollution (niveaux)
  "AQI", "NO2", "PM10", "PM2_5", "SO2",
  # Lags J-1
  "NO2_lag1","PM10_lag1","PM2_5_lag1","SO2_lag1",
  ...
  "NO2_2weeks","PM10_2weeks","PM2_5_2weeks","SO2_2weeks",
  # Macros
  "inflation_rate","unemployment_rate","log_gdp",
